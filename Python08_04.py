...
with open(output_file, 'w') as wFile:
    for key in dic:
        ...
        #para cada um dos três quadros de leitura positivos
        for frame in range(3):
            ...
            wFile.write(f"{key}-frame-{frame + 1}\n{split_seq.strip()}\n")

        #para os três quadros de leitura negativos (complemento reverso)
        rev_seq = complemento_reverso(seq_to_split)
        for frame in range(3):
            split_seq = ''
            seq_frame = rev_seq[frame:]  #ajusta o quadro de leitura conforme o valor de 'frame'
...
print(f"A sequência dividida em códons nos 6 quadros de leitura foi salva em {output_file}, gostaria de visualizar? s/n:")
